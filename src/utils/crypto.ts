...
export const encrypt = (text: string): AuthKey => {
    const cipher = crypto.createCipheriv(ALGORITHM, KEY, getIV())
    return {
...
export const saveKey = async (text: string): Promise<void> => {
    console.log(chalk.white('Saving encrypted private key to local file system'))
...
export const getKey = async (): Promise<string> => {
    console.log(chalk.white('Getting private key from local file system'))
    try {
        const f = await fs.readFile(path.join(__dirname, KEY_FILE_NAME))
        console.log(chalk.green('Key found'))
        const s = f.toString('utf8').split('|')
        return decrypt(s[0], s[1])
    } catch (e) {
        console.log(e)
        console.log(chalk.yellow('Key not found, requires input'))
    }
    return ''
}
